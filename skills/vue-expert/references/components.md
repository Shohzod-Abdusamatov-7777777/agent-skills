...
## defineModel (Vue 3.4+ - Simplified v-model)
...
```vue
<script setup lang="ts">
// Define component options without separate script block
defineOptions({
  name: 'MyComponent',
  inheritAttrs: false
})
...
<template>
  <div class="wrapper">
    <!-- Explicitly bind attrs to inner element -->
    <input v-bind="$attrs" class="input" />
  </div>
</template>

<!-- Usage - class and data-* go to input -->
<template>
  <CustomInput class="my-input" data-testid="email" />
</template>
```
...
| `defineExpose()` | Expose methods to parent refs |
...
| `useAttrs()` / `useSlots()` | Access attrs/slots in setup |
