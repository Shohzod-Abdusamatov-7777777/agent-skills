...
```typescript
// app.config.ts - Client-side config (reactive, hot-reloadable)
...
const primaryColor = appConfig.ui.primaryColor
</script>

// Runtime config vs App config:
// - runtimeConfig: sensitive data, env variables (server/client)
// - appConfig: public app settings, theming, feature flags (client only)
```
...
| `useAppConfig()` | Access app.config.ts |
